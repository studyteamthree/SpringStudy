...
- **모든 예외는 적절하게 복구되든지 아니면 작업을 중단시키고 운영자 똦는 개발자에게 분명하게 통보돼야 한다.**
...
RuntimeException(**Unchecked Exception**) | 1. Java.lang.RuntimeException 클래스를 상속한 클래스들이다.  <br>2. 명시적인 예외처리를 강제하지 않기 때문에 Unchecked Exception라고 불린다. <br> 3. 클래스 이름을 따서 런타임 에러라고도 한다. <br>4. catch 문으로 잡거나 throws로 선언하지 않아도 된다. <br> 5. 주로 프로그램 오류가 있을 때 발생하도록 의도 에러이다.(NullPointerException, IllegalArgumentException ...)
...
- 복구 불가능한 상황이 많아 API 파원에서 런타임 예외를 던지게 되었음
